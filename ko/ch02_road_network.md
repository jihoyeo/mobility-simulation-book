...
1장에서 시뮬레이터가 도로망을 따라 차량 위치를 갱신한다고 했습니다. 0장 결과의 평균 대기시간 4.1분도 차가 도로를 따라 승객에게 가는 시간을 더한 값입니다. 그 계산에 쓰인 자료가 도로망입니다.
...
- 통행수단에 맞게 엣지를 걸러 내고, 거르지 않으면 어떤 경로가 나오는지 설명합니다
- 좌표를 가장 가까운 노드에 붙이는 스냅을 KD-트리로 빠르게 만듭니다
...
0장에서 본 12,566개와 28,589개보다 훨씬 많습니다. 0장의 값은 자동차가 다닐 수 있는 도로만 남긴 뒤의 것이고, 여기서는 거르기 전의 표를 봅니다. 왜 거르는지는 2.3절에서 나옵니다.
...
```{note}
데이터를 처음 다룰 때 이런 확인을 건너뛰면, 몇 주 뒤에 원인을 알 수 없는 이상한 경로가 나옵니다. 규칙을 짐작했으면 그 자리에서 검증하는 것이 나중에 원인을 찾는 것보다 비용이 훨씬 적습니다.
```
...
전체의 절반이 넘습니다. 이걸 그대로 두고 자동차 최단경로를 구하면 어떻게 될까요. 보도나 계단을 지나는 경로가 나올 수 있습니다. 거리는 짧지만 차는 못 갑니다.
...
`modes` 인자가 방금 만든 `DRIVE` 필터에 해당합니다. `("walk",)` 로 바꾸면 보행 네트워크가 나옵니다. 엣지 표의 속도 컬럼은 자동차 기준이라 사람이 걷는 속도로는 쓸 수 없습니다. 그래서 `speed_kmh=5` 로 속도를 직접 줍니다.

```{code-cell} python
W = load_road_graph("hanam", modes=("walk",), speed_kmh=5)
...
평균 차수가 2.3입니다. 노드 하나에서 나가는 길이 평균 두 개 남짓이라는 뜻입니다. 양방향 도로를 두 줄로 저장했으므로 교차로가 아닌 노드도 나가는 길이 둘입니다.
...
1위와 2위가 세종포천고속도로의 상행과 하행입니다. 8.5km 동안 나갈 곳이 없으니 엣지 하나로 묶였습니다. 3위는 경강로 4km 구간입니다.
...
`W` 의 엣지 비용이 이미 5km/h 기준의 초이므로 15분은 900초입니다. 최단경로 알고리즘은 아직 배우지 않았으니,
