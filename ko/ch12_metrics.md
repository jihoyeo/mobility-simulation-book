...
파일 이름과 키 이름은 DTUMOS 가 내는 것을 그대로 씁니다. `trip.json` 의 한 줄에 `trip` 과 `timestamp` 가 들어 있고, `TripsLayer` 가 그 두 키를 그대로 읽습니다. 그래서 서버에서 받은 결과 디렉터리를 뷰어에 그대로 넣어도 돌아갑니다. 11장에서 직접 짠 루프의 결과도 같은 함수로 내보낼 수 있습니다. 그쪽은 도로망 위를 달리지 않으므로 통행이 직선 하나로 나옵니다. 그 직선이 곧 그 모형의 근사 수준입니다.
...
이 절이 이 책에서 파이썬이 아닌 코드를 쓰는 유일한 자리입니다. 도구는 문제에 맞춰 고릅니다.
...
웹 뷰어는 `labs/ch12_viewer/` 입니다. 이 책에서 파이썬이 아닌 코드를 채우는 유일한 자리이고, 빈칸은 다섯 줄입니다.

```bash
cd labs/ch12_viewer && npm install && npm run dev
node tools/check_viewer.mjs               # 채점은 저장소 뿌리에서
```
