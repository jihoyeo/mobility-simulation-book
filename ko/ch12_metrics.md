...
- `utilization` 0.67 — 근무 중 차량의 3분의 2가 항상 움직이고 있었습니다. 분 단위 기록에서 (운행 중 차량 ÷ 근무 차량)의 평균입니다. 일한 분을 더해 구한 11장 `summary()` 의 0.669와 소수 셋째 자리에서 다릅니다
...
```{warning}
이 함정은 프로젝트 보고서에서 가장 자주 나옵니다. "차량을 줄였는데 대기시간이 그대로였습니다"라는 결론을 보면 먼저 서비스율을 확인합니다.
```
...
곡선이 70대에서 꺾입니다. 70대까지는 차를 더 넣을 때마다 서비스율이 오릅니다. 70대에서 80대로 가면 서비스율은 0.999에서 1.0으로 거의 그대로이고 가동률만 0.78에서 0.67로 떨어집니다. 꺾이는 지점 근처가 대개 합리적인 선택입니다.
...
표로 보면 하나 더 보입니다. 90분위 대기가 60대까지 12~14분이다가 70대에서 9.9분으로 떨어집니다. 공차 비율은 60대가 0.27로 가장 높습니다. 차가 부족할 때 멀리 있는 차를 자주 보내기 때문입니다.
...
비율은 맞습니다. 공차 비율이 0.19 대 0.186입니다. 표본이 달라 절댓값은 비교할 수 없어도 비율은 비교할 수 있습니다.

```{tip}
남의 결과와 숫자가 다르면 먼저 "같은 것을 세고 있는가"를 확인합니다. 정의가 다르거나 표본이 다른 경우가 알고리즘이 틀린 경우보다 훨씬 많습니다.
```

## 12.6 시간대별로 보기

한 숫자로 요약하면 시간에 따른 변화가 사라집니다. 0장에서 엔진 결과에 썼던 `plot_record` 를 우리 루프의 기록에 그대로 씁니다. 운행 중 차량이 언제 가장 많은지, 대기 승객이 언제 쌓이는지 봅니다.
...
대기 승객은 저녁 내내 거의 0입니다. 차 80대가 남아돌기 때문입니다. 운행 차량은 밤 10시 전후가 가장 많습니다. 시간별로 숫자를 보면 이렇습니다.
...
앞 절의 pydeck 재생은 노트북 안에서 봅니다. 발표장에서 띄우기에는 불편하고, 학기가 끝나면 남지 않습니다.

DTUMOS 의 화면은 이 문제를 웹으로 풉니다. React 와 deck.gl 로 짜여 있고, 통행을 그리는 컴포넌트 하나가 1,800줄이 넘습니다. 그 안에서 "차가 움직이는 것을 보여 주는" 부분만 떼면 셋뿐입니다.

1. `TripsLayer` 에 좌표열과 시각열을 넣습니다
2. 매 프레임 `currentTime` 을 조금씩 늘립니다
3. 늘어난 시간으로 다시 그립니다

나머지 1,700여 줄은 패널, 필터, 테마, 차트입니다. 셋만 있으면 애니메이션은 돕니다.

`export_viewer` 가 뷰어가 읽을 파일을 만듭니다.
...
export_viewer(engine, "labs/ch12_viewer/public/data", sample=200)
```

파일 이름과 키 이름은 DTUMOS 가 내는 것을 그대로 씁니다. `trip.json` 의 한 줄에 `trip` 과 `timestamp` 가 들어 있고, `TripsLayer` 가 그 두 키를 그대로 읽습니다. 그래서 서버에서 받은 결과 디렉터리를 뷰어에 그대로 넣어도 돌아갑니다.

11장에서 직접 짠 루프의 결과도 같은 함수로 내보낼 수 있습니다. 그쪽은 도로망 위를 달리지 않으므로 통행이 직선 하나로 나옵니다. 직선으로 그려지는 것이 그 모형이 도로를 무시한다는 표시입니다.

뷰어는 `labs/ch12_viewer/` 에 있습니다. DTUMOS 와 같은 방식으로 실행합니다. 의존성은 deck.gl 세 개와 Vite 하나뿐이고, React 도 상태 관리 라이브러리도 쓰지 않습니다. 학생이 고치는 파일은 `src/main.js` 하나이고, 그 안의 다섯 자리가 빈칸입니다.
...
파이썬만 써 봤다면 자바스크립트 문법은 여섯 가지만 알면 됩니다. `main.js` 맨 위에 채워진 예제 함수 `tripId` 가 있고, 나머지 다섯도 같은 모양입니다.

| 파이썬 | 자바스크립트 | 뜻 |
|---|---|---|
| `d["trip"]` | `d.trip` | 객체의 값 꺼내기. `d["trip"]` 도 됩니다 |
| `return x` | `return x;` | 값을 돌려줍니다. 줄 끝에 세미콜론 |
| `a and b` | `a && b` | 둘 다 참 |
| `lambda p: p.x > 1` | `(p) => p.x > 1` | 이름 없는 함수 |
| `[p for p in ps if 조건]` | `ps.filter((p) => 조건)` | 조건에 맞는 것만 남기기 |
| `COLOR["occupied"]` | `COLOR.occupied` | 사전의 값. 키가 변수면 `COLOR[key]` |

Node 20.19 이상이 필요합니다. 처음 한 번만 설치하고, 그다음부터는 실행만 합니다.
...
`src/main.js` 를 고치고 저장하면 브라우저가 알아서 다시 그립니다. 새로고침이 필요 없어서 빈칸을 하나씩 채워 가며 확인하기 좋습니다.

파이썬으로 하지 않는 이유가 있습니다. 지도 위에서 시간을 흘리는 일은 브라우저가 훨씬 잘합니다. `TripsLayer` 는 `currentTime` 을 그래픽 카드로 바로 넘겨서, 통행 수천 개를 매 프레임 다시 계산하지 않고도 꼬리를 그립니다. 파이썬에서 같은 것을 하려면 프레임마다 좌표를 다시 만들어야 합니다.
...
이 절이 이 책에서 파이썬이 아닌 코드를 쓰는 유일한 자리입니다.

## 12.9 보고서에 무엇을 적을 것인가

프로젝트 보고서의 결과 절은 이렇게 구성합니다.
...
6번을 빼먹지 않습니다. 우리 시뮬레이터는 재배치가 없고, 소요시간을 직선거리로 근사했고, 신호 대기를 반영하지 않았습니다. 이것을 적지 않은 보고서는 결과를 실제보다 확실한 것처럼 보이게 합니다.

## 이 장의 실습

노트북 `labs/ch12_metrics.ipynb` 를 열어 함께 돌립니다.

차량 대수를 바꿔 가며 파레토 곡선을 그리고, 보고서에 그대로 넣을 표를 만듭니다.
마지막에 결과를 웹 뷰어로 내보냅니다.

```bash
jupyter lab labs/ch12_metrics.ipynb
```

웹 뷰어는 `labs/ch12_viewer/` 입니다. 이 책에서 파이썬이 아닌 코드를 채우는 유일한 자리이고, 빈칸은 다섯 자리입니다. 한 터미널에서 뷰어를 켜 둡니다.
...
채점은 새 터미널을 열어 저장소 뿌리에서 합니다.

```bash
node tools/check_viewer.mjs
```

Node 설치가 안 되면 12.7절의 pydeck 으로 같은 통행을 노트북 안에서 재생합니다. 뷰어는 그다음에 해도 됩니다.
...
- `export_viewer` 가 낸 파일을 `labs/ch12_viewer/` 프로젝트가 읽습니다. DTUMOS 프론트엔드와 같은 두 키를 씁니다
