...
문제를 표로 적으면 명확해집니다. 행이 승객, 열이 차량, 칸이 그 차가 그 승객에게 가는 데 걸리는 시간입니다. 지금은 두 점 사이 직선거리를 시속 25km 로 나눈 값을 씁니다. 이 값이 얼마나 나쁜지는 10.6절에서 봅니다.
...
승객 0은 차량 0이 가장 가깝습니다. 승객 1과 승객 2는 둘 다 차량 1이 가장 가깝습니다. 겹칩니다.
...
승객 0이 차량 0을, 승객 1이 차량 1을 가져갑니다. 승객 2는 가장 가까운 차량 1이 없어졌으므로 7분 걸리는 차량 2를 받습니다. 총 12.1분입니다.

공정해 보입니다. 먼저 부른 사람이 먼저 받으니까요. 그런데 표를 다시 보면 승객 2에게 차량 1을 주고 승객 1에게 차량 0을 주는 편이 낫습니다. **전체 대기시간의 합**이 줄어듭니다.
...
다행히 이 문제에는 이름과 해법이 있습니다. **할당 문제(assignment problem)** 이고, 헝가리안 알고리즘으로 풉니다. 승객이 두 배가 되면 계산이 여덟 배쯤 늘어나는 정도라, 경우의 수를 다 세는 것과 달리 수백 명까지 문제없습니다. `scipy` 가 구현을 제공하므로 직접 짜지 않습니다.
...
승객 0이 가장 가까운 차량 0을 양보하고 차량 2를 받습니다. 0.8분이 3.4분이 됩니다. 대신 승객 1이 차량 0(4.8분), 승객 2가 차량 1(2.6분)을 받아 총 대기가 12.1분에서 10.7분으로 줄었습니다. 한 사람이 2.5분 손해 보고 나머지 둘이 4분 가까이 이득을 본 것입니다. 이것이 이 장의 결론입니다. 먼저 부른 사람에게 가장 가까운 차를 주는 것이 전체로는 최선이 아닙니다.
...
```{tip}
그래도 탐욕이 필요한 경우가 있습니다. 승객이 수만 명이면 헝가리안도 느려집니다. 승객이 두 배면 계산이 여덟 배가 되기 때문입니다. 그럴 때는 후보를 미리 추려서(각 승객마다 가까운 차 20대만) 행렬을 작게 만듭니다. 실제 엔진의 `dispatch_top_k` 인자가 이것입니다.
```
...
무작위 점은 시 경계 밖에 떨어져 도로로 닿지 않을 수 있습니다. 3.4절에서 본 경로 없음이고, 라우팅 행렬에 무한대로 나타납니다. 그런 쌍이 있으면 다시 뽑아서 세 방법이 같은 승객·차량을 보게 합니다.
...
라우팅으로 정한 것이 가장 낫습니다. 채점 기준 자체가 라우팅 비용이므로 당연한 결과입니다.

중요한 것은 **ETA 모델이 라우팅에 얼마나 가까운가**입니다. 이 표본에서 ETA 모델은 여섯 쌍 중 셋을 라우팅과 같게 골랐고, 실제 총 대기는 34.9분으로 라우팅의 33.5분과 1.4분 차이입니다. 직선거리로 정한 배차는 50.9분으로 17분 넘게 손해입니다. 라우팅의 100분의 1 시간으로 이 정도면 배차에는 쓸 만합니다.
...
```{note}
차량이 여러 대이고 각 차에 적재 한계가 있으면 차량경로 문제(VRP)가 됩니다. 실무에서는 Google OR-Tools 가 이런 문제의 전용 솔버입니다. 좋은 해를 하나 만들고 조금씩 바꿔 가며 개선한다는 점은 2-opt 와 같습니다.
```
...
3번이 이 장에서 만든 것입니다. 나머지를 11장에서 만듭니다. 11장 첫머리에 같은 목록이 다시 나옵니다.

## 이 장의 실습

노트북 `labs/ch10_dispatch.ipynb` 를 열어 함께 돌립니다.

모든 경우를 세어 보는 완전탐색으로 정답지를 만들고, 그것으로 헝가리안 알고리즘을 검증합니다.
...
- 승객과 빈 차가 여럿 겹칠 때 할당 문제로 풀면 총 대기시간이 평균 14% 줄고, 나쁠 때는 79%까지 차이가 납니다
